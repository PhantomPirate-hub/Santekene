...
export default function MedecinDashboardPage() {
  return (
    <motion.div
      ...
      animate="visible"
    >
      <motion.h1 variants={itemVariants} className="text-3xl font-bold text-nuit-confiance mb-8">Tableau de Bord Médecin</motion.h1>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <Shortcut
          icon={<FilePlus className="w-12 h-12 text-aqua-moderne" />}
          title="Créer ordonnance"
          emoji="🧾"
        />
        <Shortcut
          icon={<BarChart2 className="w-12 h-12 text-aqua-moderne" />}
          title="Statistiques"
          emoji="📈"
        />
        <Shortcut
          icon={<FolderOpen className="w-12 h-12 text-aqua-moderne" />}
          title="DSE patient"
          emoji="📂"
        />
      </motion.div>
