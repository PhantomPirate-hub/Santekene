  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants}>
        <Card className="bg-gradient-to-r from-bleu-clair to-aqua-moderne text-white overflow-hidden">
            ...
            <div className="flex items-start justify-between">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    {doctorProfile?.structure && (
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-white/90">{doctorProfile.structure}</span>
                      </div>
                    )}
                    {doctorProfile?.location && (
                      <div className="flex items-center space-x-2">
                        <MapPin className="w-4 h-4 text-white/80" />
                        <span className="text-white/90">{doctorProfile.location}</span>
                      </div>
                    )}
                    {doctorProfile?.phone && (
                      <div className="flex items-center space-x-2">
                        <Phone className="w-4 h-4 text-white/80" />
                        <span className="text-white/90">{doctorProfile.phone}</span>
                      </div>
                    )}
                    {doctorProfile?.email && (
                      <div className="flex items-center space-x-2">
                        <Mail className="w-4 h-4 text-white/80" />
                        <span className="text-white/90">{doctorProfile.email}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="flex space-x-6">
                <div className="text-center bg-white/10 backdrop-blur-sm rounded-lg px-6 py-4">
                  ...
                  <p className="text-3xl font-bold">{doctorProfile?.patientsCount || 0}</p>
                  <p className="text-sm text-white/80">Patients</p>
                </div>
                <div className="text-center bg-white/10 backdrop-blur-sm rounded-lg px-6 py-4">
                  <Stethoscope className="w-6 h-6 text-white mx-auto mb-2" />
                  <p className="text-3xl font-bold">{doctorProfile?.consultationsCount || 0}</p>
                  <p className="text-sm text-white/80">Consultations</p>
                </div>
              </div>
            </div>
          </CardContent>
